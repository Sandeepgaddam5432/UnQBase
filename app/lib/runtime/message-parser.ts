...
const BOLT_QUICK_ACTIONS_OPEN = '<UnQBase-quick-actions>';
const BOLT_QUICK_ACTIONS_CLOSE = '</UnQBase-quick-actions>';
    ...
    while (i < input.length) {
      if (input.startsWith(BOLT_QUICK_ACTIONS_OPEN, i)) {
        ...
        if (actionsBlockEnd !== -1) {
          ...
          // Find all <UnQBase-quick-action ...>label</UnQBase-quick-action> inside
          const quickActionRegex = /<UnQBase-quick-action([^>]*)>([\s\S]*?)<\/UnQBase-quick-action>/g;
          let match;
          const buttons = [];
...
function createQuickActionElement(props: Record<string, string>, label: string) {
  const elementProps = [
    ...
    'data-UnQBase-quick-action="true"',
...
function createQuickActionGroup(buttons: string[]) {
  return `<div class=\"__boltQuickAction__\" data-UnQBase-quick-action=\"true\">${buttons.join('')}</div>`;
}
